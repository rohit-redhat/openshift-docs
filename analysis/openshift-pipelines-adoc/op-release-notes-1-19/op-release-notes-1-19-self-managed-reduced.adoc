...
* With this update, cache archives are compressed using `Gzip` before being uploaded. This reduces object storage costs and accelerates data transfer, especially for large caches such as `Go` modules.
...
+
--
.Example of enabling the StatefulSet ordinals for the `Chains` controller
[source,yaml]
----
apiVersion: operator.tekton.dev/v1alpha1
kind: TektonChains
...
* Before this update, end-to-end (E2E) tests were unstable due to GitOps comments being incorrectly associated with canceled pipeline runs. This behavior caused intermittent test failures and reduced reliability in CI/CD pipelines. With this update, GitOps comments are no longer mixed with canceled pipeline runs, resulting in stable and predictable E2E tests.
