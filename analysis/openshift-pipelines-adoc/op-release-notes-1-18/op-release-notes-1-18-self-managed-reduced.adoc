...
# ...
----
`default-maximum-resolution-timeout`:: Specify a global maximum timeout for resolution requests. The default value is `1m`.
`fetch-timeout`:: Specify a timeout that is used for bundle resolution requests.
...
# ...
----
`pipelinesascode.tekton.dev/on-path-changed`:: Matches if changes occur in the `docs` directory.
`pipelinesascode.tekton.dev/on-path-change-ignore`:: Matches if no changes occur in the `.github` directory.
...
# ...
----
`pipelinesascode.tekton.dev/on-comment`:: The example configures the `/hello-world` command. When you use this command on a pushed commit, for example, on the `main` branch, the `on-comment-pr` pipeline run is triggered.
...
* Before this update, when using {pac}, a `/test branch:<branch>` command where a branch name contained parts that could be interpreted as other commands did not work properly. For example, in the command `/test branch:a/testbranch` the branch name contains `/test`, and this command would fail. With this update, only the first `/test` substring in a comment is considered a command to prevent any malfunction.
